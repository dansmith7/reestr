...
async function ensureDefaultStages(p) {
  const existing = (await p.query('SELECT id, name FROM stages')).rows;
  const stageIds = Object.fromEntries(existing.map((row) => [row.name, row.id]));

  for (let i = 0; i < DEFAULT_STAGES.length; i++) {
    const name = DEFAULT_STAGES[i];
    if (stageIds[name]) {
      continue;
    }
    ...
    stageIds[name] = r.rows[0].id;
  }

  return stageIds;
}

async function seedMissingDefaultProjects(p, stageIds) {
  for (const [name, status, timing, comment] of DEFAULT_PROJECTS) {
    const existing = await p.query(
      'SELECT id FROM projects WHERE name = $1 AND archived = false LIMIT 1',
      [name]
    );
    if (existing.rows.length > 0) continue;

    const pr = await p.query('INSERT INTO projects (name) VALUES ($1) RETURNING id', [name]);
...
export async function getRegistryCounts(p) {
...
export async function seedDefaultRegistryData() {
  schemaReady = false;
  const p = await ensureSchema();
  const stageIds = await ensureDefaultStages(p);
  await seedMissingDefaultProjects(p, stageIds);
  return getRegistryCounts(p);
}
...
export async function serializeProject(p, project) {
  return serializeProjectAt(p, project, null);
}

export async function serializeProjectAt(p, project, snapshotDate) {
  ...
  const latestRes = await p.query(
    `SELECT sl.*, s.name AS stage_name, s.position AS stage_position
     ...
     WHERE sl.project_id = $1
       AND ($2::text IS NULL OR sl.created_at < ($2::date + interval '1 day'))
  ...
  const referenceTime = date
    ...
    : Date.now();
  const weeksSince =
    Math.max(0, referenceTime - new Date(latest.created_at).getTime()) /
    (1000 * 60 * 60 * 24 * 7);
