...
async function seedDefaultProjectsIfEmpty(p, stageIds) {
  const { rows } = await p.query(
    'SELECT COUNT(*)::int AS c FROM projects WHERE archived = false'
  );
...
export async function ensureSchema() {
  ...
  await seedDefaultProjectsIfEmpty(p, stageIds);
